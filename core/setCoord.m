...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    if size(coordData,1) == 1 
        coordData = vector2coord(coordData');
    elseif size(coordData,2) == 3 
        coordData = coordData';
    elseif size(coordData,2) == 1
         coordData = vector2coord(coordData);
    else
        error('Useful_func:coordDataError','coordData must be a vector or n by 3 array.');
    end
    pdbStruct = setAttribute(pdbStruct,selectedAtoms,'coord',coordData);
end